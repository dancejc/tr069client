...
#endif
          r = recv(soap->socket, s, (int)n, soap->socket_flags);//recv message by jc
#ifdef PALM
...
{ if (!soap_tag_cmp(key, "Host"))
  ...
  { if (!soap_tag_cmp(val, "keep-alive"))
      ...
      soap->keep_alive = 0;
    /* by jc modify begin */
    else if (!soap_tag_cmp(val, "TE, close"))
    	soap->keep_alive = 0;
    /* by jc modify end */
  }
#ifndef WITH_LEAN
...
{ register const char *s;
  ...
  if ((err = soap->fposthdr(soap, "Host", soap->tmpbuf)))
    return err;
  //if ((err = soap->fposthdr(soap, "User-Agent", "gSOAP/2.8")))//by jc
  if ((err = soap->fposthdr(soap, "User-Agent", "JZ TR69C/1.0")))//by jc
    return err;
...
{ register int err;
  ...
  //if ((err = soap->fposthdr(soap, "Server", "gSOAP/2.8"))//by jc
  if ((err = soap->fposthdr(soap, "Server", "JZ TR69C/1.0"))//by jc
...
{ register size_t count;
  ...
  { register int n = soap->mode;
    ...
    if ((n & SOAP_IO) != SOAP_IO_FLUSH)
      ...
      //jc modify begin
 			if(!strncmp(soap->buf, "HTTP/1.1 200 OK", 15))
 			{
 				char tmp[256];
 				bzero(tmp, 0);
 				tmp[0] = 'h';
 				strcpy(tmp + 1, soap->endpoint + 1);
		    if ((soap->error = soap->fpost(soap, tmp, soap->host, soap->port, soap->path, soap->action, count)))
		      return soap->error;
	    }
	    else
	    {
...
{ soap_wchar c;
  ...
  *soap->endpoint = '\0';
  ...
  { soap_mode m = soap->imode;
    ...
    /* by jc modify begin */
    /* This change because TR069 POST EMPTY, gsoap can`t detected it. */
    /* I change this for cpe can close when working under KEEPALIVE mode. */
    if(soap->length == 0)
    {
    	DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Receive a HTTP zero length packet.\n"));//fserveloop
    	/* sometime ITMS return "XXX, close", GSOAP not support this format.
    	   So, force set the keep-alive invalid.*/
    	//soap_clr_omode(soap, SOAP_IO_KEEPALIVE);
    	soap->keep_alive = 0;//temp, maybe case any problem.
    	
    	soap->error = TR069_EMPTY;
...
int
SOAP_FMAC2
soap_send_empty_request(struct soap *soap, const char *endpoint)
{
	/*if ((soap->mode & SOAP_IO) != SOAP_IO_STORE && !(soap->mode & SOAP_ENC_XML))
  { register int n = soap->mode;
    soap->mode &= ~(SOAP_IO | SOAP_ENC_ZLIB);
    if ((n & SOAP_IO) != SOAP_IO_FLUSH)
      soap->mode |= SOAP_IO_BUFFER;*/
    //soap->mode = 139281;//temp
    register int n = soap->mode;
    soap->mode = SOAP_IO_BUFFER;
    soap->bufidx = soap->buflen = 0;
    //soap_flush(soap);
    //soap->buf[0] = '\0';//temp
    //soap->tmpbuf[0] = '\0';//temp
    //soap->msgbuf[0] = '\0';//temp
		soap_set_endpoint(soap, endpoint);
  	soap->count = 0;
  	if (!soap->fpost(soap, soap->endpoint, soap->host, soap->port, soap->path, soap->action, 0) && !soap_end_send(soap))
  ...
  soap->fpost(soap, soap->endpoint, soap->host, soap->port, soap->path, soap->action, 0);
  //if(soap->error = soap->fpost(soap, soap->endpoint, soap->host, soap->port, soap->path, soap->action, 0))
  	//return soap->error;
  soap->host[0] = '\0';
  soap->port = 0;
  soap->endpoint[0] = '\0';
